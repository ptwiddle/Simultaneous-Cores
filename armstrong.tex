...
In the remaining section of the paper, we begin to apply the lattice point viewpoint toward $q$ and $(q,t)$ analogs of $\Cat_{a,b}$.   




nctions on $\mathcal{C}_{a,b}$, so that the $q$ and $(q,t)$ analogs are sums over the lattice points in

Section \ref{sec:qcat} investigates connections between the lattice point $q$ rational Catalan numbers
$$\Cat_{a,b}(q)=\frac{1}{[a+b]_q}\binom{a+b}{a}$$
the obvious $q$ analog of rational Catalan numbers.

In particular, one might hope that $\Cat_{a,b}(q)$




numbers, which have an algebraic definition
