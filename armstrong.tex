...
Rational Catalan numbers, and their $q$ and $(q,t)$ analogs, are a natural generalization of Catalan numbers that, apart from their intrinsic combinatorial interest, appear in the study of Hecke algebras \cite{GG}, affine Springer varieties \cite{LS}, and compactified Jacobians of singular curves \cite{EM1, EM2}. It is a theorem of Anderson that simultaneous core partitions are counted by rational Catalan numbers.
Our first result is to reprove Anderson's theorem by identifying simultaneous core partitions with lattice points in a rational simplex.  The connection between rational Catalan numbers and this simplex is not new; it appears for instance in \cite{LS, GMV}.    We are not aware of any previous application of Ehrhart theory using this fact, however.

 After this identification is made, many other results follow quite naturally.  
...
\end{theorem}
Our first result is a new proof of Theorem \ref{thm:anderson} using the geometry of lattice points in rational polyhedra.  This new viewpoint easily extends to prove other results; chief among them a proof of Armstrong's conjecture:
...
is also a polynomial of degree $d+ n$. Ehrhart reciprocity also extends\footnote{Although well known to experts, apparently this was first used (without proof) in \cite{CJM}; a proof now appears in \cite{AB}}: $$L(f,P,-t)=(-1)^nL(f, -P^\circ, t)$$
