...
If $a$ and $Wb$ are relatively prime, the number of
...
The first construction, which we call the \emph{positive abacus}, gives a bijection between $a$ core partitions and $\N^{a-1}$.  Anderson's original proof used the positive abacus to give a bijection between $(a,b)$-cores and $(a,b)$-Dyck paths, which were already known to be counted by $\Cat_{a,b}$.

The second construction, which we call the \emph{signed abacus}, gives a bijection between $a$-core partitions and the $a-1$ dimensional lattice 
...
Upon first hearing Armstrong's conjecture, I immediately had the following
...
The false hope gives a strategy to prove Anderson's theorem and Armstrong's conjecture.  Ehrhart theory would imply that for fixed $b$, $|\mathcal{C}_{a,b}|$ would be a polynomial of degree $b-1$ in $a$.  Thus, for any fixed $b$, we could prove Anderson's conjecture by checking only finitely many values.  

But it is clear from the definition that if we fix $b$, then for $a$ relatively prime to $b$, then we have $\Cat_{a,b}$ is a polynomial of degree $b-1$ in $a$.  Thus, proving Anderson's theorem is reduced to showing these two polynomials are equal, which only requires checking it for finitely many values.
...
The False Hope is not quite true.  One minor tweak is that as we vary $a$, the polytope is not only scaled, by also changed by a linear transformation.  These transformations preerve the number of lattice points and the quadratic form $Q$ giving the size of the partitions, and so does not pose any real difficulties.

More troubling is the fact that the polytope is not an integral polytope, but rather only a rational polytope.   
...
different residue classes are in general mysterious.  Perhaps the most studied aspect, \emph{period collapse} (see \cite{Haase} and references), where the quasipolynomial is in fact a polynomial, is in some ways orthogonal to our situation.
