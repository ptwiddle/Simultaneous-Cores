...
We first deal with both orbifold cosets in Chamber I.  The rays pointing inward to Chamber I at $x_1$ are $-v_1$and $v_2-v_1$; reading from Lemma \ref{ }, we see that these rays of $(\ell, \sk^\prime)$ weights $(-1, 2)$ and $(1, 0)$, respectively.  Thus the denominator from the Chamber I cosets will be $(1-q^{-1}t^2)(1-q)$.  


To find the numerator, we find the closest points from each orbifold coset in chamber 1 $x_1=(-2b/9, b/9, b/9)=(-2k/3-2/9-\delta/9,k/3+1/9+\delta/9, k/3+1/9+\delta/9)$.  

First, assume that $b=3k+1$, that is, $\delta=0$.  Then, the closest point in coset containing $(-1/3, 0, 1/3)$, this point is $(-2k/3+1/3, k/3-1/3, k/3)$, in the other coset it is $(-2k/3, k/3-1/3, k/3+1/3)$; the point closest to $x_1$ in Chamber 2 is $(-2k/3-1/3, k/3, k/3+1/3)$.

However, if we now increase $b$ by 1, making $\delta=0$, we see that the point $(-2k/3-1/3, k/3, k/3+1/3)$ now has $x_2-x_0=k+2/3=b/3$; thus it is no longer strictly in chamber II but on the wall of chamber I.  
