...
We apply these results to rational Catalan combinatorics.  Using Ehrhart theory, we reprove Anderson's theorem \cite{anderson} that there are $(a+b-1)!/a!b!$ simultaneous $(a,b)$-cores, and using Euler-Maclaurin theory we prove Armstrong's conjecture \cite{AHJ} that the average size of an $(a,b)$-core is $(a+b+1)(a-1)(b-1)/24)$.
...
There are two main tools in the proofs of these theorems: the abacus construction, Ehrhart / Euler-Maclaurin theory.  We breifly recall these ideas before giving a high-level overview of the proof.
...
To explain the method used to prove Theorems \ref[], we begin with the following \begin{FalseHope}
Fix $b$.  Then under the signed abacus construction, the set of $(a,b)$-cores maps to $a\Delta$, for some $(b-1)$-dimensional integral polytope $\Delta$.
\end{FalseHope}
The false hope provides an obvious strategy to prove Anderson's theorem and Armstrong's conjecture.  If the false hope were true, Ehrhart theory would imply that for fixed $b$, $|\mathcal{C}_{a,b}|$ would be a polynomial of degree $b-1$ in $a$.  But it is clear from the definition of $\Cat_{a,b}$ that if we fix $b$, then for $a$ relatively prime to $b$ $\Cat_{a,b}$ is a polynomial of degree $b-1$ in $a$.  Thus, proving Anderson's theorem is reduced to showing these two polynomials are equal, which only requires checking it for finitely many values.

Furthermore, it is a theorem that thee size of a $b$ core partition is a quadratic function on the lattice.  Thus, if the dream were true Euler-Maclaurin theory would give that the total size of all $(a,b)$ cores was a polynomial of degree $b+1$, and proving Armstrong's conjecture could be done by checking it for finitely any values.

\subsubsection{}

The False Hope is not quite true, but the strategy outlined above is still the one we wfollow.  One minor tweak needed to the False hope, is that as we vary $a$, the polytope is not only scaled, by also changed by a linear transformation.  These transformations preserve the number of lattice points and the quadratic form $Q$ giving the size of the partitions, and so does not pose any real difficulties.

More troubling is the fact that the polytope is not an integral polytope, but rather only a rational polytope -- that is, also $\Delta$ is not a lattice polytope, there is some $k\in\Z$ so that $k\Delta$ is a lattice polytope.   
...
We see that since $\Cat_{a,b}$ is defined only for $a$ and $b$ relatively prime, it fits nicely into the quasipolynomial framework.  For fixed $a$, and for $b$ in a fixed residue class mod $a$, we have $\Cat_{a,b}$ is polynomial; it just so happens that for any residue class relatively prime to $a$, these polynomials are identical.

Such ``accidental'' equalities between the polynomials for different residues happens somewhat frequently in Ehrhart theory, but in general is rather mysterious and not much is known.  Perhaps the most studied manifestation of this is, \emph{period collapse} (see \cite{Haase} and references), where the quasipolynomial is in fact a polynomial, does not directly relate to our situation.

Although the general strategy outlined above can be carried forth with quasipolynomials instead of polynomials, it is much more difficult as many more values must be checked, and I could not find a way to prove Theorems just using quasipolynomiality.  

Instead, we found that in this case there is a rather clear explanation of the ``accidental'' equality of the different residue classes, which we now explain.

The polytope of $a,b$ cores turns out to be an $a-1$ dimensional simplex with a $\Z/a\Z$ symmetry.  This symmetry does not, however, fix the lattice $\Lambda_a$: rahter, there is a larger lattice $\Lambda_a^\prime$ containing $\Lambda_a$, with $[\Lambda_a:\Lambda^\prime_a]=a$. When $b$ is relatively prime to $a$, this action does not have any fixed points, and each orbit in $\Lambda_a^\prime$ contains exactly one element of $\Lambda_a$.  Thus, the number of points in $\Delta\cap \Lambda_a$ is exactly $1/a$ of the number of points in $\Delta\cap \Lambda_a^\prime$.
